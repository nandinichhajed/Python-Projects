...
def key_event(*args): # the function is going to be triggered when we click any key on the keybord.
    if disp.get() == '0':
        disp.delete(0, END)
...
disp = Entry(root, font="Verdana 20", fg="black", bg="gray", bd=0, justify=RIGHT, insertbackground="#abbab1", cursor="arrow")
disp.bind("<Return>", btneq_clicked)
disp.bind("<Escape>", btnc_clicked)
disp.bind("<Key-1>", key_event)# we created thisbinding here because when we open the calc we see zero so to make any data visible.
...
disp.insert(0, '0') # inserting default zero at starting
disp.focus_set() # setting focus to entry
disp.pack(expand=TRUE, fill=BOTH) # packing means showing it all to the display.
...
#expand = true means when we expand the grid the button should also expand.
...
btnm = Button(btnrow2, text="-", font="Segoe 23", relief=GROOVE, bd=0, command=btn_minus_clicked, fg="white", bg="#333333")
...
btnml = Button(btnrow3, text="*", font="Segoe 23", relief=GROOVE, bd=0, command=btn_multiply_clicked, fg="white", bg="#333333")
